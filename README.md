...
The tests cover schedules and exact budgets, scalar solvers, safe result
round-trips, deterministic manifests, SP and μP model construction,
optimizer-group scaling, validation-only selection, censoring, and CPU smoke
training.
...
```bash
python scripts/run_scale_transfer.py smoke
python scripts/run_scale_transfer.py snapshot-source \
  --output /path/to/dropout-source-snapshot \
  --slurm-script scripts/bridges2/coord_check_h100.sbatch \
  --slurm-script scripts/bridges2/run_phase_h100.sbatch
```
...
```bash
python scripts/run_scale_transfer.py plan \
...
```bash
python scripts/run_scale_transfer.py coord-check \
  --run-dir "$RUN_DIR" \
...
```bash
python scripts/run_scale_transfer.py run \
...
```bash
python scripts/run_scale_transfer.py select \
  --run-dir results/scale_transfer
...
python scripts/run_scale_transfer.py aggregate \
  --run-dir results/scale_transfer

python scripts/make_scale_transfer_figures.py \
...
sbatch -A PROJECT --export=ALL \
  "$PROJECT_DIR/scripts/bridges2/coord_check_h100.sbatch"
```
...
sbatch -A PROJECT --array=0-3%4 \
  ...
  "$PROJECT_DIR/scripts/bridges2/run_phase_h100.sbatch"
