...
python scripts/run_benchmark_suite.py smoke
python scripts/run_benchmark_suite.py cost
...
DATA_ROOT=$DATA_ROOT,VENV_DIR=$VENV_DIR \
  scripts/bridges2/prepare_benchmark_data.sbatch

# Check every cache loads before spending any GPU time on it.
python scripts/run_benchmark_suite.py verify-data --data-root $DATA_ROOT

# Then the whole three-stage chain, with Slurm dependencies between stages.
./scripts/bridges2/submit_benchmark_chain.sh -A YOUR_ALLOCATION
```
...
PROJECT_DIR=$PROJECT_DIR,RUN_DIR=$RUN_DIR,VENV_DIR=$VENV_DIR,DATA_ROOT=$DATA_ROOT \
  scripts/bridges2/run_benchmark_h100.sbatch
```

Progress and results:

```bash
python scripts/run_benchmark_suite.py status --run-dir $RUN_DIR
python scripts/run_benchmark_suite.py aggregate --run-dir $RUN_DIR
```
...
./scripts/bridges2/submit_depth12_pilot_chain.sh -A YOUR_ALLOCATION
```
...
RUN_DIR="$RUN_DIR" VENV_DIR="$VENV_DIR" \
  ./scripts/bridges2/sync_benchmark_wandb.sh
```
