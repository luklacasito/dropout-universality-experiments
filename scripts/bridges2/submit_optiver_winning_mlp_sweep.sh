...
python scripts/run_optiver_winning_mlp_sweep.py plan \
...
canary_raw=$(sbatch --parsable "${SBATCH_ARGS[@]}" \
  ...
  "${HERE}/run_optiver_winning_mlp_h100.sbatch")
...
PYTHONPATH=${PROJECT_DIR}/src python scripts/run_optiver_winning_mlp_sweep.py \
...
echo "Status: python scripts/run_optiver_winning_mlp_sweep.py status --run-dir ${RUN_DIR}"
